...
for(s in 1:S)
{
	...
	lr <- lr+sum(
		...
		sum(dnorm(theta.p, pmn.theta, psd.theta, log=TRUE)),
...
```{r eval=FALSE}
save(THETA.post, file="data_output/sim/NormalPrior/FE.THETA.post.S=220000.RData")
save(ACCEPT.Theta.count, file="data_output/sim/NormalPrior/FE.ACCEPT.Theta.count.S=220000.RData")
```

```{r eval=TRUE}
load("data_output/sim/NormalPrior/FE.THETA.post.S=220000.RData")
load("data_output/sim/NormalPrior/FE.ACCEPT.Theta.count.S=220000.RData")
```
