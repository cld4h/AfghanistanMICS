...
thin=50
thin2=2
N_THIN=(S-B)/(thin*thin2)
DIC_array<-array(dim=N_THIN)
for(i in 1:N_THIN)
{
  DIC_i <- 0
  for(j in 1:m)
  {
    DIC_i <- DIC_i + sum(-2 * dpois(y.list[[j]], lambda=exp(matmul(Z.list[[j]], GAMMA.post.mat[[j]][i,]) + matmul(X.list[[j]],as.matrix(THETA.post.mat[,i]) )),log=TRUE))
  }
  DIC_array[i] <- DIC_i
}

DIC_bar = mean(DIC_array)
D_mean<-0
for(j in 1:m)
{
  D_mean <- D_mean + sum(-2 * dpois(y.list[[j]], lambda=exp(matmul(Z.list[[j]], apply(GAMMA.post.mat[[j]],2,mean)) + matmul(X.list[[j]],as.matrix(apply(THETA.post.mat, 1, mean)) )),log=TRUE))
}
DIC <- 2*DIC_bar - D_mean
# 111513.5
```
