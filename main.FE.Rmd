...
library(ggplot2)
BP.df_province <- data.frame(x =1:35,
                 F = c(qEA[2,], qE[2,])[index],
                 L = c(qEA[1,], qE[1,])[index],
                 U = c(qEA[3,], qE[3,])[index],
              name = dnames[index])
abc <- ggplot(BP.df_province, aes(x = x, y = F)) +
...
save(BP.df_province, file=paste(IMAGEOUT,"BP.df_province.RData",sep=""))
...
for (n in map$name){
	...
	eCEB.CI.width <- append(eCEB.CI.width, df$U[which(df$name==n)]-df$L[which(df$name==n)])
}
...
dev.print(pdf, paste(IMAGEOUT, "E_province_map.pdf",sep=""),width=16,height=16)
```
...
BP.df_windex3 <- data.frame(x =1:3,
                 F = c(qE[2,])[index],
                 L = c(qE[1,])[index],
                 U = c(qE[3,])[index])
abc <- ggplot(BP.df_windex3, aes(x = x, y = F)) +
...
save(BP.df_windex3, file=paste(IMAGEOUT,"BP.df_windex3.RData",sep=""))
...
BP.df_women_edu <- data.frame(x =1:2,
                 F = c(qE[2,])[index],
                 L = c(qE[1,])[index],
                 U = c(qE[3,])[index])
...
BP.df_HH_edu <- data.frame(x =1:2,
                 F = c(qE[2,])[index],
                 L = c(qE[1,])[index],
                 U = c(qE[3,])[index])
abc <- ggplot(BP.df_HH_edu, aes(x = x, y = F)) +
...
save(BP.df_HH_edu, file=paste(IMAGEOUT,"BP.df_HH_edu.RData",sep=""))
...
for(i in 1:N_THIN)
{
  DIC_i <- sum(-2 * dpois(y, lambda=exp(matmul(X,as.matrix(THETA.post.mat[,i]) )),log=TRUE))
  DIC_array[i] <- DIC_i
}

DIC_bar = mean(DIC_array)
D_mean <- sum(-2 * dpois(y, lambda=exp(matmul(X,as.matrix(apply(THETA.post.mat, 1, mean)) )),log=TRUE))
