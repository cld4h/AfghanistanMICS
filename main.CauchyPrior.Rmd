...
#
#
## This is to setup a different variance for differnet coefficient.
...
plot(x, yc, type = "l", lty = 1, ylab = "Prior densities", xlab = "", cex.lab = 2, col = 1, ylim=c(0,10))

yc <- dcauchy(x, location = 0, scale = 0.03936)
lines(x, yc, lty = 1, col = 2)

dev.print(pdf, paste(IMAGEOUT, "CauchyPriorForPredictors.pdf",sep=""),width=8,height=8)
```
...
for(s in 1:S)
{
  ...
  dlr <- sum( dcauchy(theta.p, pmn.theta, psd.theta, log=TRUE)) -sum(dcauchy(THETA, pmn.theta, psd.theta, log=TRUE))
	lr <- lr + dlr
  ################debug###############
...
```{r eval=FALSE}
RESULTPATH = "./data_output/BHP.Cauchy/"
...
dev.print(pdf,
	paste(IMAGEOUT,"Cauchy.THETA.post.boxplot.pdf",sep=""),
	width=16,height=12)
...
dev.print(pdf,
	paste(IMAGEOUT,"Cauchy.THETA.post.pdf",sep=""),
	width=16,height=12)
...
dev.print(pdf,
	paste(IMAGEOUT,"Cauchy.ACF.THETA.post.pdf",sep=""),
	width=16,height=12)
...
dev.print(pdf,
	paste(IMAGEOUT,"Cauchy.SIGMA.post.pdf",sep=""),
	width=16,height=12)
...
dev.print(pdf,
	paste(IMAGEOUT,"Cauchy.ACF.SIGMA.post.pdf",sep=""),
	width=16,height=12)
```
