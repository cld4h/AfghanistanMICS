...
for(s in 1:S)
{
	...
	lr <- lr+sum(
		...
		sum( log(dCSS(theta.p, df_spike, pmn.theta, psd.theta, df_slab, pmn.theta, scale_slab, p_spike))),
		-sum(log(dCSS(THETA, df_spike, pmn.theta, psd.theta, df_slab, pmn.theta, scale_slab, p_spike)))
	)
	#print("lr is")
	#print(lr)
	if( log(runif(1))<lr ) {
...
```{r eval=FALSE}
save(THETA.post, file="data_output/sim/SSPrior/FE.THETA.post.S=220000.RData")
save(ACCEPT.Theta.count, file="data_output/sim/SSPrior/FE.ACCEPT.Theta.count.S=220000.RData")
```

```{r eval=TRUE}
load("data_output/sim/SSPrior/FE.THETA.post.S=220000.RData")
load("data_output/sim/SSPrior/FE.ACCEPT.Theta.count.S=220000.RData")
```
