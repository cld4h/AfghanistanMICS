...
#
#
# # This is to setup a different variance for different coefficient.
...
for(s in 1:S)
{
  ...
  dlr <-  sum( dlaplace(theta.p, pmn.theta, psd.theta, log=T)) -sum(dlaplace(THETA, pmn.theta, psd.theta, log=T))
	lr <- lr + dlr
  ################debug###############
...
```{r eval=FALSE}
RESULTPATH = "./data_output/BHP.Laplace/"
...
dev.print(pdf,
	paste(IMAGEOUT,"Laplace.THETA.post.boxplot.pdf",sep=""),
	width=16,height=12)
...
dev.print(pdf,
	paste(IMAGEOUT,"Laplace.THETA.post.pdf",sep=""),
	width=16,height=12)
...
dev.print(pdf,
	paste(IMAGEOUT,"Laplace.ACF.THETA.post.pdf",sep=""),
	width=16,height=12)
...
dev.print(pdf,
	paste(IMAGEOUT,"Laplace.SIGMA.post.pdf",sep=""),
	width=16,height=12)
...
dev.print(pdf,
	paste(IMAGEOUT,"Laplace.ACF.SIGMA.post.pdf",sep=""),
	width=16,height=12)
```
...
Xs[,44] <- c(2, 1) # 2 for Urban, 1 for Rural
...
Xs <- as.matrix(c(1, Xs))
Xs <- matrix(rep(Xs, 2), nrow = 2, byrow = T)
Xs[,46] <- c(0, 1) # 0 for Male, 1 for Female
...
Xs <- as.matrix(c(1, Xs))
Xs <- matrix(rep(Xs, 2), nrow = 2, byrow = T)
Xs[,10] <- c(0, 1) # 0 for Male, 1 for Female
...
Xs <- as.matrix(c(1, Xs))
Xs <- matrix(rep(Xs, 2), nrow = 2, byrow = T)
Xs[,6] <- c(1, 2) # 0 for Male, 1 for Female
