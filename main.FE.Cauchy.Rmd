...
for(s in 1:S)
{
	...
	lr <- lr+sum(
		...
		-wgt * dpois(y,
			...
			sum( dcauchy(theta.p, pmn.theta, psd.theta, log=T)),
			-sum(dcauchy(THETA, pmn.theta, psd.theta, log=T))
	)
	#print("lr is")
	#print(lr)
	if( log(runif(1))<lr ) {
...
```{r eval=FALSE}
save(THETA.post, file="data_output/sim/CauchyPrior/FE.THETA.post.S=220000.RData")
save(ACCEPT.Theta.count, file="data_output/sim/CauchyPrior/FE.ACCEPT.Theta.count.S=220000.RData")
```

```{r eval=TRUE}
load("data_output/sim/CauchyPrior/FE.THETA.post.S=220000.RData")
load("data_output/sim/CauchyPrior/FE.ACCEPT.Theta.count.S=220000.RData")
```
